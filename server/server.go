...
import (
	...
	shushApi "github.com/tivvit/shush/shush-api"
...
func Server(confFile string) {
	...
	shushApi.SetBackend(sb)
	shushApi.SetGenerator(s)
	go func() {
		log.Printf("API Server starting at %s", cfg.Api.Address)
		log.Fatal(http.ListenAndServe(cfg.Api.Address, shushApi.NewRouter()))
		wg.Done()
	}()
	wg.Wait()
}
